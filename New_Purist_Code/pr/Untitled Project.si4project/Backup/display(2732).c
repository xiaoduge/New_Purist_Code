...
定量取水ON        |            OFF  |      ON           |           OFF          |      ON         |     ON |      ON         |  ON            |ON
...
void Disp_ValveCtrl(STATE_STRU *state)
{
   ...
   case DISPLAY_STATE_IDLE:
       switch(state->ucSub)
        ...
        case DISPLAY_SUB_STATE_IDLE_FLUSH:
            ...
            RelayLogicCtrl(STM32F4_VALVE1_24V_EN,FALSE);
   ...
   case DISPLAY_STATE_CIRCULATION:
       RelayLogicCtrl(STM32F4_VALVE3_24V_EN,FALSE);
       RelayLogicCtrl(STM32F4_VALVE1_24V_EN,TRUE);
       RelayLogicCtrl(STM32F4_VALVE2_24V_EN,FALSE);
   ...
   case DISPLAY_STATE_NORMAL_TAKING_WATER:
       RelayLogicCtrl(STM32F4_VALVE3_24V_EN,TRUE);
       RelayLogicCtrl(STM32F4_VALVE1_24V_EN,FALSE);
       RelayLogicCtrl(STM32F4_VALVE2_24V_EN,TRUE);
       RelayLogicCtrl(STM32F4_LAMP_24V_EN,TRUE);
       RelayLogicCtrl(STM32F4_PUMP_24V_EN,TRUE);
       // ADD FOR V2
       RelayLogicCtrl(STM32F4_VALVE4_24V_EN,FALSE);
...
void Disp_ReadInfo(void)
{
    ...
    if (INVALID_CONFIG_VALUE_SHORT == Display.info.PACKLIFEL)
    {
    ...
    if (INVALID_CONFIG_VALUE_SHORT == Display.info.UVLIFEHOUR)
    {
    ...
    VOS_LOG(VOS_LOG_DEBUG,"ulUFCleanTime = %d",Display.ulUFCleanTime);
...
void Disp_ReadQtwVolume(void)
{
    ...
    if (INVALID_CONFIG_VALUE_SHORT == Display.usQtwWq)
    {
        Display.usQtwWq = 0;
    }
...
void Disp_Move2DstState(void)
{
    ...
    {
    case DISPLAY_STATE_QUANTITY_TAKING_WATER:
        RelayLogicCtrl(STM32F4_WATER_LED_N,FALSE);
        ucPfm4Uv = TRUE;
        ucMeas = TRUE;
        break;
    case DISPLAY_STATE_NORMAL_TAKING_WATER:
    ...
    case DISPLAY_STATE_CIRCULATION:
        break;
    case DISPLAY_STATE_NORMAL_TAKING_WATER:
        RelayLogicCtrl(STM32F4_WATER_LED_N,TRUE);
        break;
    case DISPLAY_STATE_QUANTITY_TAKING_WATER:
        break;
    case DISPLAY_STATE_UF_CLEAN:
    ...
    if (Display.bit1AuthState)
    {
        ...
        bleCmdSerialWrite(SAPP_CMD_DATA | 0x80,dataBuf,4);        
    }
...
{
    int result = GetInputLogicStatus(STM32F4_WATER_LEVEL_N); // late implement
...
{
    system_untimeout(&Display.to4qtw);
...
void Disp_uf_clean_msg_cb(void)
{
    ...
    {
        system_untimeout(&Display.to4UfClean);
        return;
    }
...
void Disp_Move2UFPreClean(void)
{
   ...
   // add supervisor time
   system_timeout(30000,SYS_TIMER_ONE_SHOT,30000,Disp_uf_pre_clean_cb,&Display,&Display.to4UfPrePare);
...
void Disp_Move2UFClean(void)
{
   ...
   // add supervisor time
   system_timeout(1000,SYS_TIMER_PERIOD,1000,Disp_uf_clean_cb,&Display,&Display.to4UfClean);
...
void Disp_Move2QtwTakingWater(void)
{
   ...
   system_timeout(Display.spec.qtw.time4tw,SYS_TIMER_ONE_SHOT,Display.spec.qtw.time4tw,Disp_qtw_cb,&Display,&Display.to4qtw);
}
...
void Disp_Move2Ntw(void)
{
   ...
   system_timeout(DISP_MINUTE2MS,SYS_TIMER_PERIOD,DISP_MINUTE2MS,Disp_ntw_cb,&Display,&Display.to4qtw);
}
...
void Disp_degass_msg_cb(void)
{
    ...
    {
        system_untimeout(&Display.to4Degass);
...
void Disp_Move2Degass(void)
{
   ...
   system_timeout(1000,SYS_TIMER_PERIOD,1000,Disp_degass_cb,&Display,&Display.to4Degass);
...
void Disp_engSetDrawPack(int xoff,int yoff,u16 charColor,u16 bkColor)
{
    ...
    else
    {
        ...
        case DISP_LAN_GERMANY:
            ...
            break;
        default:
            sprintf((char *)Config_buff,"%d 天  %d升    ",Display.cfg.cfg2.PACKLIFEDAY,Display.cfg.cfg2.PACKLIFEL);
            break;
        }
...
void Disp_engSetDrawFilter(int xoff,int yoff,u16 charColor,u16 bkColor)
{
    ...
    {
    case DISP_LAN_ENGLISH:
        sprintf((char *)Config_buff,"FILTER LIFE:%d DAYS    ",Display.cfg.cfg2.FILTERLIFE);
        break;
    case DISP_LAN_GERMANY:
        sprintf((char *)Config_buff,"FILTER LIFE:%d TAGE    ",Display.cfg.cfg2.FILTERLIFE);
        break;
    default:
        sprintf((char *)Config_buff,"过滤器寿命 :%d 天    ",Display.cfg.cfg2.FILTERLIFE);
        break;
    }
...
void Disp_EngSetInitPage(uint8_t ucCurLine,uint8_t ucCurCol,uint8_t ucLineOffSet)
{
    ...
    for (ucLineIdx = 0; ucLineIdx < DEF_MIN(DISPLAY_MAX_LINE_PER_PAGE,iLines);ucLineIdx++)
    {
        ...
        case DISP_ES_ITEM_UF_DISINFECTION:
            if (DEV_TYPE_V1 == Display.cfg.devtype)
            {
...
void Disp_ReadConfig(void)
{
...

    {
        ...
        {
            if (INVALID_CONFIG_VALUE_SHORT == Display.cfg.cfg2.TEMP_MAX[iChl])
            {
                Display.cfg.cfg2.TEMP_MAX[iChl] = 55; // celsius
            }
        
            if (INVALID_CONFIG_VALUE_SHORT == Display.cfg.cfg2.TEMP_MIN[iChl])
            {
    ...
    if (INVALID_CONFIG_VALUE_SHORT == Display.cfg.cfg2.PACKLIFEDAY)
    {
    ...
    if (INVALID_CONFIG_VALUE_SHORT == Display.cfg.cfg2.UNIT)
    {
    ...
    if (INVALID_CONFIG_VALUE_SHORT == Display.cfg.cfg3.devtype)
    {
    ...
    if (INVALID_CONFIG_VALUE_SHORT == Display.cfg.cfg3.usUFCleanTime)
    {
        Display.cfg.cfg3.usUFCleanTime = 30; // DAY
    }

    if (INVALID_CONFIG_VALUE_SHORT == Display.cfg.cfg3.usUFFlushTime)
    {
        Display.cfg.cfg3.usUFFlushTime = 30; // second
    }

    if (INVALID_CONFIG_VALUE_SHORT == Display.cfg.cfg3.usAutoUFFlushTime)
    {
        Display.cfg.cfg3.usAutoUFFlushTime = 30; // min
    }

    if (INVALID_CONFIG_VALUE_SHORT == Display.cfg.cfg3.usIdleCirPeriod)
    {
...
#ifdef TOC
    if (Display.ulCalcirtime  > 1000*(Display.cfg.usTocOxiTime + Display.cfg.usUFCleanTime*2))
    {
        Display.ulCalcirtime -= 1000*(Display.cfg.usTocOxiTime + Display.cfg.usUFCleanTime*2);
    }
    else
    {
        Display.ulCalcirtime = 1000*(Display.cfg.usTocOxiTime + Display.cfg.usUFCleanTime*2) + 3*DISP_MINUTE2MS;
    }
#endif
...
void DispMakeQueryCmd(void)
{
    ...
    bleCmdSerialWrite(SAPP_CMD_AT,(uint8_t *)cmd,strlen(cmd));
}
...
void Disp_Adc_proc(void)
{
  ...
  if (adc)
  {
      ...
      }
      {
          ...
          Disp_Calc_Conductivity(adc->ucChl);
          ...
          // log information
          //UartCmdPrintf(VOS_LOG_DEBUG,"CHL%d,C:%dmv,T:%dmv",iChl+1,value1,value2);
    ...
    case RF_CMD_ACCESS: // access info
        {
            ...
            if (len <= CmdLen - 2)
            {
                ...
                VOS_LOG(VOS_LOG_DEBUG,"RF_CMD_ACCESS %s&%s \r\n",szName,szPass);
...
void Disp_Init(void)
{
   ...
   Display.ucChlMask |= 1<<DISPLAY_SENSOR_2; 
    ...
    system_timeout(1000,SYS_TIMER_PERIOD,100,Disp_adc_cb,&Display,&Display.to4Adc);
