...
typedef enum
{
   ...
   DISPLAY_SUB_PAGE_US_SET_INIT_TIME,
...
typedef struct
{
    UINT32 PACKLIFEDAY;   //´¿»¯Öù:  0~999 DAYS    0~9999L 
    UINT32 FILTERLIFE;    //¹ýÂËÆ÷ÊÙÃü :0~999DAYS
    UINT32 UVLIFEDAY;     // 0~999 DAYS
    UINT16 UVLIFEHOUR;    // 0~9999Hr (unit 10min)
    UINT16 PACKLIFEL;     //´¿»¯Öù:   0~9999L 
}DISP_INFO_STRU;
...
typedef struct
{
    ...
    system_timeo to4qtw; // timer for quantity&normal taking water

    system_timeo to4cir; // timer for circulation duration

    
    system_timeo to4UfPrePare; // timer for idle state supervisor 

    system_timeo to4UfClean; // timer for idle state supervisor 

    system_timeo to4TOC; // timer for idle state supervisor 

    system_timeo to4TOCMeas; // timer for TOC fast measurement 

    system_timeo to4Degass; // timer for DE gass
    
    system_timeo to4Adc; // timer for sensor
...
#define DISP_RUNTIME_INFO_OFFSET (DISP_QTW_VOLUME_OFFSET + (sizeof(uint16_t) +1)/2*2)
