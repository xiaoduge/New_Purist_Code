...
typedef struct {
  // The 31 LSB's of 'val' indicate the total item size, including the header, in 8-bit bytes.
  unsigned int len : 31;
...
static __align(32)  osalMemHdr_t theHeap[MAXMEMHEAP / OSALMEM_HDRSZ] __attribute__((at(0X10000000 + OSAL_MEMORY_START_ADDRESS))); 
...
#if OSALMEM_PROFILER
  (void)memset(theHeap, OSALMEM_INIT, MAXMEMHEAP);
#endif
...
{
  halIntState_t cpu_sr = 0;
  osalMemHdr_t *tmp = osal_mem_alloc(1);

  HAL_ASSERT((tmp != NULL));
  OS_ENTER_CRITICAL();  // Hold off interrupts.
...
#endif /* DPRINTF_OSALHEAPTRACE */
{
  ...
  if ( hdr != NULL )
  {
    ...
    (void)memset((uint8 *)(hdr+1), OSALMEM_ALOC, (hdr->hdr.len - OSALMEM_HDRSZ));
#endif
...
#endif /* DPRINTF_OSALHEAPTRACE */
{
  ...
  (void)memset((uint8 *)(hdr+1), OSALMEM_REIN, (hdr->hdr.len - OSALMEM_HDRSZ) );
#endif
#if OSALMEM_METRICS
  memAlo -= hdr->hdr.len;
  blkFree++;
